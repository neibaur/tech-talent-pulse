...
The repository is currently in Phase 6. Stack Overflow raw ingestion, daily analytics transformation code, read-only dashboard API endpoints, static API documentation, and explicit local demo data support are allowed. Dashboard UI, GitHub ingestion, Kafka/event streaming, authentication/security stack, deployment infrastructure, and additional external clients should wait for later scoped phases.
